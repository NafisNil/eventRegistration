...
export default function Create() {
  const handleSubmit = (formData: FormData) => {
    router.post('/leaderships', formData, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => {
        Swal.fire({
          ...
          text: 'The leadership entry was created successfully.',
          timer: 2000,
    ...
    <>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-linear-to-r from-emerald-900 via-emerald-800 to-teal-900 p-6 text-white shadow-sm">
          ...
          <h1 className="mt-2 text-2xl font-bold">Create Leadership</h1>
          <Link href="/leaderships" className="text-sm text-emerald-200 hover:underline">
            Back to list
          </Link>
        </div>

        <LeadershipForm submitLabel="Create Leadership" onSubmit={handleSubmit} />
      </div>
    </>
  );
}
