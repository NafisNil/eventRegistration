...
export default function Index({ leaderships = [] }: IndexProps) {
  const handleDelete = (id: number) => {
    Swal.fire({
      ...
      router.delete(`/leaderships/${id}`, {
        preserveScroll: true,
        onSuccess: () => {
          Swal.fire({
            icon: 'success',
            title: 'Leadership deleted',
            text: 'The leadership entry was deleted successfully.',
            timer: 2000,
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3 rounded-2xl bg-linear-to-r from-emerald-900 via-emerald-800 to-teal-900 p-6 text-white shadow-sm">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold">Leadership Management</h1>
          </div>

          <Link
            href="/leaderships/create"
        ...
        <div className="overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm">
          {leaderships.length === 0 ? (
            <div className="p-8 text-center text-slate-500">No leadership entries found yet.</div>
          ) : (
                ...
                <tbody className="divide-y divide-emerald-100">
                  {leaderships.map((leader) => (
                    <tr key={leader.id} className="align-top">
                      <td className="px-4 py-4 font-medium text-slate-800">{leader.name}</td>
                      <td className="px-4 py-4 text-slate-700">{leader.role ?? '—'}</td>
                      <td className="px-4 py-4 text-slate-700">{leader.ministry ?? '—'}</td>
                      <td className="px-4 py-4">
                        {leader.logo ? (
                          <img
                            src={`/storage/${leader.logo}`}
                            alt={leader.name}
                            className="h-16 w-16 rounded-lg border border-emerald-200 bg-emerald-50 object-cover"
                          />
                        ) : (
                          '—'
                        )}
                      ...
                      <td className="px-4 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/leaderships/${leader.id}/edit`}
